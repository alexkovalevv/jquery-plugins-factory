...
(function($) {
	...
	if( !$.aikaComponent.sbtns.okShare ) {
		$.aikaComponent.sbtns.okShare = {};
	}
	$.aikaComponent.sbtns.okShare.idx = 100;
	...
	button.prepareOptions = function() {
		$.aikaComponent.sbtns.okShare.idx++;
		this.idx = $.aikaComponent.sbtns.okShare.idx;
	};

	button.counterInit = function() {
		var self = this;
		$(document).bind($.aikaCore.tools.hash('ok-counter-ready-' + this.idx), function(e, idx, number) {
	...
	button.rewriteVkMethod = function() {
		...
		if( window.ODKL.updateCount ) {
			$.aikaComponent.sbtns.okShare.oldShareCallback = window.ODKL.updateCount;
		}
	...
	button.getCounterByVkMethod = function(idx, number) {
		if( idx > 100 ) {
			$(document).trigger($.aikaCore.tools.hash('ok-counter-ready-' + idx), [idx, number]);
		} else {
			if( $.aikaComponent.sbtns.okShare.oldShareCallback ) {
				$.aikaComponent.sbtns.okShare.oldShareCallback(idx, number);
			}
		}
	};
	...
	$.aikaComponent.sbtns.buttons["odnoklassniki-share"] = button;
