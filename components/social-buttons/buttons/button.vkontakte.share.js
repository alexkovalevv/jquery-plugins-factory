...
(function($) {
	...
	if( !$.aikaComponent.sbtns.vkShare ) {
		$.aikaComponent.sbtns.vkShare = {};
	}
	$.aikaComponent.sbtns.vkShare.idx = 100;
	...
	button.prepareOptions = function() {
		$.aikaComponent.sbtns.vkShare.idx++;
		this.idx = $.aikaComponent.sbtns.vkShare.idx;
	};

	button.counterInit = function() {
		var self = this;
		$(document).bind($.aikaCore.tools.hash('vk-counter-ready-' + this.idx), function(e, idx, number) {
	...
	button.rewriteVkMethod = function() {
		...
		if( window.VK.Share.count ) {
			$.aikaComponent.sbtns.vkShare.oldShareCallback = window.VK.Share.count;
		}
	...
	button.getCounterByVkMethod = function(idx, number) {
		if( idx > 100 ) {
			$(document).trigger($.aikaCore.tools.hash('vk-counter-ready-' + idx), [idx, number]);
		} else {
			if( $.aikaComponent.sbtns.vkShare.oldShareCallback ) {
				$.aikaComponent.sbtns.vkShare.oldShareCallback(idx, number);
			}
		}
	};

	button.getIcon = function() {
		return '<svg class="plusonet-svg generator-elem-active" viewBox="-90 213.9 435 435">' +
	...
	$.aikaComponent.sbtns.buttons["vkontakte-share"] = button;
